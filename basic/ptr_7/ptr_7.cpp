// ptr_7.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
// unique_ptr续
...
int main()
{
	...
	// shared_ptr，删除器不同，但指向类型一样的shared_ptr，可以放到同一个容器里.
	// unique_ptr, 删除器不同，那么就等于整个unique_ptr类型不同。这种类型不同的unique_ptr智能指针是没有办法放到同一个容器里的
