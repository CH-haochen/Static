// class_11.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int main()
{
	...
	//int &b{ a }; //b绑定到a
	//int &c; //错误，引用必须要初始化
	//int &c = 1; //不可以，左值引用不能帮到到右值，必须绑定到左值
	...
	//总结：
	//返回左值引用的函数，连同赋值，下标，解引用和前置递增递减运算符（--i）,都是返回左值表达式的例子；我们可以将一个左值引用绑定到这类表达式结果上
	//返回非引用类型的函数，连同算数，不安席，位以及后置递增递减运算符（i--）,都生成右值，不能将一个左值引用绑定到这类表达式上，
	//	但是我们可以将一个const引用和右值引用绑定到这类表达式上
