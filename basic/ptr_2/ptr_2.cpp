// ptr_2.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
// new、delete探秘，只能指针概述，share_ptr概述
...
int main()
{
	...
	//1.2.2 delete干了两个事情：(a)调用析构函数， (b)释放内存(调用operator delete()来释放内存)
	...
	//结论：如果一个对象（内置对象int,float,包括类对象），使用new[]来分配内存，却用单独的delete来释放内存，那么这个对象需要满足的条件是
	...
	//二 智能指针总述
	//c++ 标准库有四种智能指针 std::
	//（a）auto_ptr(c++98); unique_ptr; shared_ptr; weak_ptr
