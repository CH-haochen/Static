// basic_6.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
//string 类型介绍
...
int main()
{
	//二、stirng类型简介：c++标准库中的类型，代表一个可变长字符串
	...
	//四 string对象上的操作
	//（a）判断是否为空empty()，返回布尔值
	//（b）size()/length()：返回字节/字符数量 代表改字符串的长度。unsigned int
	//（c）s[n] 返回s中的第n个字符(n是个整型值)，n代表的是一个位置，位置从0开始，到.size()-1
	//		如果用下标n超过这个范围的内容，或者本来就是一个空字符串，也用s[n]访问，会产生不可预测的结果
	//（d）s1+s2 字符串的连接，返回连接后的结果，得到一个新的string对象
	//（e）s1 = s2 字符串对象赋值，用s2里面的内容取代原来s1里面的内容
	//（f）s1 == s2 判断连个字符串是否相等。大小写敏感
	//		相等：长度相同，字符全相同
	//（g）s1 != s2 判断亮字符串是否不相等
	//（h）s.c_str() 返回一个字符串s中的内容指针。返回一个指向正规c字符串的指针常量，也就是以\0结尾的
	//	这个函数的引入是为了与c语言兼容，在c语言中没有string类型，所以我们得通过string类对象的c_str()成员函数把string对象转换成
	//	c语言中的字符串格式
	//（i）读写string对象
	// stirng s1;
	// cin >> s1;
