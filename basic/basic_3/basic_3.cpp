// basic_3.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int main()
{
	...
	//（2）堆：只要不超出你实际拥有的物理内存，也在操作系统运行你能够分配的最大内存大小 之内，都可以分配给你
	//			分配速度比栈慢，你可以随时用malloc/new来分配，free/delete 非常灵活

	//malloc和free:在c语言中，用malloc和free从堆中来分配和释放内存使用。malloc和free是函数
	//malloc(memory allocation)：动态 内存分配
	//一般形式：
	//void *malloc(int Numbytes); //Numbytes:要分配的字节数。分配成功则返回指向被分配内存的指针，分配失败则返回NULL。 void * 万能指针，可以强制转换成其他任何指针
	//当这段分配内存不使用的时候，你应该使用free()函数来将内存释放掉，供其他地方使用
	//free:
	//void free(void *FirstByte):将之前用malloc分配的内存空间还给程序（操作系统），也就是说释放了这块内存，这样这块内存就被系统回收，并在需要的时候由系统分配出去再给其他使用
	...
	//new和delete:是运算符（标识符）。c++中，就用new/delete分配和释放内存，不再使用malloc和free
	//new 一般使用格式
	//（1） 指针变量名 = new 类型标识符;
