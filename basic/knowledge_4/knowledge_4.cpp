// knowledge_4.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
...
//模板函数：目的 要把收到的参数以及这些参数相对于的类型不变的转发给其他函数,这就叫转发
...
int main()
{
	...
	//完美转发：让外贸可以写接受任意类型实参的函数模板，并将其转发到目标函数，目标函数会接收到与转发函数所接收到的完全相同（左值行，右值性都要保持)的实参

	//三 std::forward
	//c++11新函数，专门为抓饭而存在。要么返回一个左值，要么返回一个右值
	//发挥作用的条件：调用模板函数，模板函数参数是万能引用类型，模板函数负责转发
	//std::forward 就是按照参数本来的类型转发, 保持原始实参的类型(左值/右值)


	//四: std::move和std::forward的区别
	//forward是强制把一个左值转换成一个右值。但是如果你实参就是左值，forward就什么也不干
	//move: 无条件强制类型转换
